import { Search, Settings, Filter, BarChart3, CheckSquare, Network, ClipboardCheck, BookOpen, Smile, Target, Wallet } from "lucide-react";
...
export const Header = ({ onSearch, onFilterClick }: HeaderProps = {}) => {
  ...
  const isOnHabits = location.pathname === '/habits';
  const isOnLedger = location.pathname === '/ledger';
  const [showSearch, setShowSearch] = useState(false);
  const [searchValue, setSearchValue] = useState("");

  const handleSearch = () => {
    if (isOnDashboard || isOnFramework || isOnReview || isOnDiary || isOnMood || isOnHabits || isOnLedger) {
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between">
          ...
          <Button
            ...
            title="Habit Tracker"
          >
            <Target className="h-4 w-4 group-hover:text-cool-blue transition-all duration-200 group-hover:scale-110" strokeWidth={2} />
          </Button>
          <Button
            variant="ghost"
            size="icon"
            ...
            title="Trelix Ledger"
          >
            <Wallet className="h-4 w-4 group-hover:text-cool-blue transition-all duration-200 group-hover:scale-110" strokeWidth={2} />
          </Button>
          <Button
            variant="ghost"
            size="icon"
        ...
        {showSearch && !isOnDashboard && !isOnFramework && !isOnReview && !isOnDiary && !isOnMood && !isOnHabits && !isOnLedger && (
