...
const AddEntryForm = ({ onClose, onSave, editingEntry, prefillAmount }: Omit<AddEntryModalProps, 'isOpen'>) => {
  const { theme } = useTheme();

  // Form state initialized from props
  const [type, setType] = useState<'income' | 'expense'>(editingEntry?.type || 'expense');
  
  // Derived categories
  const categories = type === 'expense' ? EXPENSE_CATEGORIES : INCOME_CATEGORIES;
  
  const [category, setCategory] = useState(editingEntry?.category || categories[0].name);
  const [amount, setAmount] = useState(editingEntry ? editingEntry.amount.toString() : (prefillAmount ? prefillAmount.toString() : ''));
