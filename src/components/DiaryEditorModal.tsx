...
const DiaryEditorForm = ({
  onClose,
  onSave,
  initialEntry,
  date,
}: Omit<DiaryEditorModalProps, 'isOpen'>) => {
  const [content, setContent] = useState(initialEntry?.content || '');
  const [tagInput, setTagInput] = useState('');
  const [tags, setTags] = useState<string[]>(initialEntry?.tags || []);
  const [mood, setMood] = useState<DiaryEntry['mood']>(initialEntry?.mood || null);
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            <DiaryEditorForm 
              key={initialEntry ? initialEntry.id : 'new-entry'}
              onClose={onClose}
              onSave={onSave}
